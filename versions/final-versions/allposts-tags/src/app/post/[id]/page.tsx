...
function MarkdownRenderer({ content }: { content: string | null }) {
    if (!content) return <p className="text-gray-400">No hay contenido disponible</p>;
...
export default function PostPage() {
  ...
  useEffect(() => {
    ...
    const fetchPostData = async () => {
      ...
      try {
        ...
        setError("Ocurrió un error al cargar la publicación")      } finally {
        setLoading(false)
      }
    }
  ...
  const handleVote = async () => {
    ...
    } catch (err) {      console.error("Error al gestionar el voto:", err)
    }
  }
